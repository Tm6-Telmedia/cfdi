...
searchForm.addEventListener('submit', (e) => {
    ...
    if (value.trim()) {
        fetch('/tm/assets/json/indexing.json', { keepalive: true }).then(response => response.json())
            .then(data => {
                ...
                results = data.filter(element => {
                    ...
                    return searchWords.every(word => (fieldsToSearch.includes(word) || fieldsToSearch.includes(aSingular(word))));
                });
                ...
                if (results) {
                    ...
                    const pagAll = (results.length / 10).toFixed(1);
                    pag(pagAll);
                    showPage(1);
...
function showPage(pageNumber) {
    ...
    pageItems.forEach(element => {
        let skuNumerPart = "";
        if(element?.id_num_parte && element?.id_sku) {
    ...
    pag(Math.ceil(results.length / 10), pageNumber);
}
