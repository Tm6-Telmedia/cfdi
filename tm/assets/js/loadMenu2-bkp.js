      ...
      <div class="card_inner">
        <div class="card_top">
          <a href="${fullPath}index.shtml">
            <img src="${resolved}"
                 alt="${altText}" class="product-img img-responsive"  width="400" height="240">
          </a>
        </div>
...
function compareMosaico() {
  ...
  const url = `/productos/comparador.shtml?skus=${encodeURIComponent(skus.join(','))}&path=${encodeURIComponent(rutaBase)}`;
...
document.addEventListener("error", function(e) {
  if (e.target.tagName === "IMG") {
    ...
    if (img.src.includes("@2x")) {
      const normalSrc = img.src.replace("@2x", "");
      console.warn(`Imagen no encontrada ${normalSrc}`);
      img.src = normalSrc;
    }
  }
}, true);
...
const obs = new MutationObserver(mutations => {
  mutations.forEach(m => {
    m.addedNodes.forEach(node => {
      if (node.tagName === "IMG" && node.src.includes("@2x")) {
        node.addEventListener("error", () => {
          node.src = node.src.replace("@2x", "");
        });
      }
    });
  });
